...
class LittleMan {
  constructor({ world, color, G, addScore, setGameOver }) {
    this.world = world;
    this.color = color;
    this.G = G;
  ...
  createBody() {
    ...
    const headSegment = (this.headSegment = new THREE.Mesh(
      headGeometry,
      material
    ));
    ...
    const bodyBottomGeometry = new THREE.CylinderBufferGeometry(
      headSize * 0.9,
      this.width / 2,
      headSize * 2.5,
      16
    ...
    const bodyCenterGeometry = new THREE.CylinderBufferGeometry(
      headSize,
      headSize * 0.9,
      headSize,
      16
    ...
    bodyGeometry.merge(
      new THREE.Geometry().fromBufferGeometry(bodyCenterGeometry)
    );
    bodyGeometry.merge(
      new THREE.Geometry().fromBufferGeometry(bodyBottomGeometry)
    );

    // 缩放控制
    const translateY = (this.bodyTranslateY = headSize * 1.5);
    const bodyScaleSegment = (this.bodyScaleSegment = new THREE.Mesh(
      bodyGeometry,
      material
    ));
  ...
  jump() {
    ...
    if (!currentProp && startX === target.x && startZ === target.z) {
      ...
      const { rangeR, rangeH } = computeObligueThrowValue(
        v0,
        theta * (Math.PI / 180),
        G
      );
      const { jumpDownX, jumpDownZ } = computePositionByRangeR(
        rangeR,
        start,
        target
      );
      ...
      // 落地后的回调
      const yDownCallBack = () => {
        const currentInfos = currentProp.computePointInfos(
          width,
          jumpDownX,
          jumpDownZ
        );
        const nextInfos = nextProp.computePointInfos(
          width,
          jumpDownX,
          jumpDownZ
        );
  ...
  fall(currentInfos, nextInfos) {
    ...
    let hitValidator =
      validateProp && getHitValidator(validateProp.body, direction, isForward);
