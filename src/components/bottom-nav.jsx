    ...
    <div className="w-full h-screen">
      {location.pathname !== "/" && (
        <img
          ...
          className="w-full h-screen fixed bottom-0 left-0 z-0"
        />
      )}
      <Outlet />
      {/* bottom navbar */}
      <div className="w-full flex items-center justify-evenly p-1 fixed bottom-0">
        {nav_list.map((item) => (
          ...
          >
            <div
              className={String.raw`flex flex-col relative items-center transition duration-300 ${
                location.pathname === item.url && "translate-y-[-1.5rem]"
              }`}
            >
              <img
                className="w-[42.6px] h-[45.3px]"
                src={
                  location.pathname === item.url ? item.yesLogo : item.noLogo
                }
              />

              <span
                className={String.raw`text-xl font-Nishiki font-bold ${
            ...
            <img
              className={String.raw`w-[40.4px] h-[22px] transition duration-300 bottom-[-1.5rem] scale-[0.001] absolute ${
                location.pathname === item.url &&
                " translate-y-[-1.5rem] scale-[1]"
              }`}
              src="/arrow.svg"
            />
          </Link>
